...
e__all_event_functions <- function(outer_env = totem) {
  ...
  action_compare <- function(session_name, current_row, view_objects, outer_env, obj_env, table_type) {
    ...
    if (!file.exists(pinned_path)) {
      err_dialog <- RGtk2::gtkMessageDialog(parent = outer_env[[session_name]]$windows$main_window, flags = "destroy-with-parent", type = "error", buttons = "close", "No data is currently pinned for comparison across sessions.")
      err_dialog$run()
      RGtk2::gtkWidgetDestroy(err_dialog)
      return()
    }
    ...
    if (inherits(pinned, "try-error")) {
      err_dialog <- RGtk2::gtkMessageDialog(parent = outer_env[[session_name]]$windows$main_window, flags = "destroy-with-parent", type = "error", buttons = "close", "Failed to read the pinned comparison file. Try pinning the data again.")
      err_dialog$run()
      RGtk2::gtkWidgetDestroy(err_dialog)
      return()
    }
    ...
    current <- list(dataset = outer_env[[session_name]]$sas_file_basename, keys = comp_info$keys, data = comp_info$data)
    ...
    if (length(pinned$keys) == 1 && length(current$keys) == 1) {
      ...
      msg <- paste0("Cannot compare data: The grouping columns do not match.\n\nPinned column(s): ", paste(pinned$keys, collapse = ", "), "\nAttempted comparison column(s): ", paste(current$keys, collapse = ", "))
      err_dialog <- RGtk2::gtkMessageDialog(parent = outer_env[[session_name]]$windows$main_window, flags = "destroy-with-parent", type = "error", buttons = "close", msg)
      err_dialog$run()
      RGtk2::gtkWidgetDestroy(err_dialog)
      return()
    }
    ...
    outer_env$u__df_view(merged_df, title_str, height = 400, width = 800)
  }
  ...
  i__all_event_functions[["General"]][["Open Context Menu"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (is.null(view_objects$event) == F) {
      ...
      RGtk2::gtkMenuPopup(obj_env$menubar[["base"]], button = click_btn, activate.time = RGtk2::gdkEventGetTime(view_objects$event))
    }
  }
  i__all_event_functions[["General"]][["Bob"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    ...
    w["title"] <- "Bob"
    if (rand == party_rand) RGtk2::gtkContainerAdd(w, RGtk2::gtkImageNewFromFile(file.path(system.file("images", package = "jaw"), "party_loading.gif")))
    else if (rand == left_rand) RGtk2::gtkContainerAdd(w, RGtk2::gtkImageNewFromFile(file.path(system.file("images", package = "jaw"), "left_loading.gif")))
    else if (rand == evil_rand) RGtk2::gtkContainerAdd(w, RGtk2::gtkImageNewFromFile(file.path(system.file("images", package = "jaw"), "evil_loading.gif")))
    else if (rand == blurry_rand) RGtk2::gtkContainerAdd(w, RGtk2::gtkImageNewFromFile(file.path(system.file("images", package = "jaw"), "blurry_loading.gif")))
    else RGtk2::gtkContainerAdd(w, RGtk2::gtkImageNewFromFile(file.path(system.file("images", package = "jaw"), "loading.gif")))
  ...
  i__all_event_functions[["Copy"]][["if then"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { outer_env$copy_if_then(session_name, obj_env$table_objects_list$current_row, obj_env$df_obj) }
  i__all_event_functions[["Copy"]][["if then do"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { outer_env$copy_if_then_do(session_name, obj_env$table_objects_list$current_row, obj_env$df_obj) }
  ...
  i__all_event_functions[["Copy"]][["Column full"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { obj_env$df_obj$copy_full(obj_env$table_objects_list$current_row$column) }
  i__all_event_functions[["Copy"]][["Column filtered"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { obj_env$df_obj$copy_filter(obj_env$table_objects_list$current_row$column) }
  ...
  i__all_event_functions[["Copy"]][["Vector Column full"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { obj_env$df_obj$copy_full(obj_env$table_objects_list$current_row$column, vector = T) }
  i__all_event_functions[["Copy"]][["Vector Column filtered"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) { obj_env$df_obj$copy_filter(obj_env$table_objects_list$current_row$column, vector = T) }
  i__all_event_functions[["Copy"]][["Row"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    ...
    row_data <- current_row$row[, clean_cols, drop = FALSE]
  ...
  i__all_event_functions[["Copy"]][["Data Columns"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    current_data <- obj_env$df_obj$current_data()
    cross_tab_names <- setdiff(colnames(current_data), c("r__", "n", "freq", "lines", "nchar"))
    clipr::write_clip(current_data[, cross_tab_names, drop = FALSE], allow_non_interactive = T)
  ...
  i__all_event_functions[["Filter"]][["Add to Main Filter"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (isTRUE(obj_env$is_full_data_table)) outer_env$u__add_before_filter_full_data(session_name, obj_env$table_objects_list$current_row) else outer_env$u__add_before_filter(session_name, obj_env$table_objects_list$current_row)
  }
  i__all_event_functions[["Filter"]][["Add to Main Filter Exclude"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (isTRUE(obj_env$is_full_data_table)) outer_env$u__add_before_filter_full_data(session_name, obj_env$table_objects_list$current_row, exclude = T) else outer_env$u__add_before_filter(session_name, obj_env$table_objects_list$current_row, exclude = T)
  }
  i__all_event_functions[["Filter"]][["Add to Main Filter (no combining)"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (isTRUE(obj_env$is_full_data_table)) outer_env$u__add_before_filter_full_data(session_name, obj_env$table_objects_list$current_row, combine = F) else outer_env$u__add_before_filter(session_name, obj_env$table_objects_list$current_row, combine = F)
  }
  ...
  i__all_event_functions[["Filter"]][["Add grepl to Main Filter"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    ...
    clean_col <- if (!grepl("^[a-zA-Z][a-zA-Z0-9]*$", col)) paste0("`", col, "`") else col
  ...
  i__all_event_functions[["Summarize"]][["Trigger Value Summary"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    view_objects$event_mapping[["Trigger Value Summary"]](session_name, col_name)
  }
  i__all_event_functions[["Summarize"]][["Trigger Value Summary with Group By"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    view_objects$event_mapping[["Trigger Value Summary with Group By"]](session_name, col_name)
  }
  i__all_event_functions[["Summarize"]][["Trigger Value Summary with Unique By"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    view_objects$event_mapping[["Trigger Value Summary with Unique By"]](session_name, col_name)
  }
  ...
  i__all_event_functions[["Organize"]][["Add Column to select"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    ...
    if (st != "") {
      ...
      if (col_to_toggle %in% current_cols) current_cols <- setdiff(current_cols, col_to_toggle) else current_cols <- c(current_cols, col_to_toggle)
  ...
  i__all_event_functions[["Organize"]][["Move column before"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (isTRUE(obj_env$is_meta_table)) {
      fake_current_row <- list(column = obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T])
      outer_env$move_column(0, session_name, fake_current_row)
    } else outer_env$move_column(0, session_name, current_row)
  }
  i__all_event_functions[["Organize"]][["Move column after"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    ...
    } else outer_env$move_column(1, session_name, current_row)
  }
  i__all_event_functions[["Organize"]][["Add Count to df"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    outer_env$u__add_count_to_df_summary(session_name, col_name)
  }
  i__all_event_functions[["Organize"]][["Format by Column"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    RGtk2::gtkEntrySetText(outer_env[[session_name]]$format_by_entry, col_name)
  ...
  i__all_event_functions[["Organize"]][["Add'l format by Column"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    col_name <- if (isTRUE(obj_env$is_meta_table)) obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T] else current_row$column
    RGtk2::gtkEntrySetText(outer_env[[session_name]]$format_by_entry2, col_name)
    outer_env[[session_name]]$data_view_list$slot1_list$full_table$update(outer_env[[session_name]]$data2)
    RGtk2::gtkWidgetHide(outer_env[[session_name]]$data_view_list$slot2_box)
  }
  i__all_event_functions[["Organize"]][["Pin for Comparison"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    tbl_type <- if (isTRUE(obj_env$is_meta_table)) "Meta Table" else if (isTRUE(obj_env$is_value_table)) "Summary Table" else "Full Data Table"
    action_pin(session_name, current_row, view_objects, outer_env, obj_env, tbl_type)
  }
  i__all_event_functions[["Organize"]][["Compare with Pinned"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    tbl_type <- if (isTRUE(obj_env$is_meta_table)) "Meta Table" else if (isTRUE(obj_env$is_value_table)) "Summary Table" else "Full Data Table"
    action_compare(session_name, current_row, view_objects, outer_env, obj_env, tbl_type)
  }
  i__all_event_functions[["Organize"]][["Freeze/Unfreeze Column"]] <- function(session_name, current_row, view_objects, outer_env = totem, obj_env = inner_env) {
    if (isTRUE(obj_env$is_meta_table)) {
      col_name <- obj_env$df_obj$current_data()[current_row$row_i, "variable", drop = T]
      outer_env[[session_name]]$data_view_list$slot1_list$full_table$freeze_column(col_name)
    } else obj_env$df_obj$freeze_column(current_row$column)
  }
