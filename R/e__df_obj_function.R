...
e__df_obj_function <- function(box, outer_env = totem,obj_env=inner_env) {
  ...
  copy_full <- function(pass_columns = NULL, vector = F) {
    if (is.null(pass_columns) == T) {
      ...
      if (totem$settings_list$maximize) outer_env$u__show_toast(session_name, "Data copied to clipboard")
    } else {
      if (vector == F) {
  ...
  copy_dataset_layout <- function() {
    ...
    clipr::write_clip(x, allow_non_interactive = T)
    if (totem$settings_list$maximize) outer_env$u__show_toast(session_name, "Layout copied to clipboard")
  }
  ...
  copy_keep <- function() {
    ...
    clipr::write_clip(x, allow_non_interactive = T)
    if (totem$settings_list$maximize) outer_env$u__show_toast(session_name, "Keep statement copied to clipboard")
  }
  ...
  copy_label <- function() {
    ...
    clipr::write_clip(x, allow_non_interactive = T)
    if (totem$settings_list$maximize) outer_env$u__show_toast(session_name, "Label statement copied to clipboard")
  }
  ...
  copy_length <- function() {
    ...
    clipr::write_clip(x, allow_non_interactive = T)
    if (totem$settings_list$maximize) outer_env$u__show_toast(session_name, "Length statement copied to clipboard")
  }
